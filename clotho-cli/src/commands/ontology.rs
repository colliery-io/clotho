...
pub fn run_get(args: OntologyGetArgs, json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let row = entity_store
        .get(&args.id)?
        .ok_or_else(|| format!("Entity not found: {}", args.id))?;

    let ontology = ontology_store.get(&args.id)?;

    if json {
        println!("{}", serde_json::to_string_pretty(&ontology)?);
    } else {
        println!("{} — {} ({})", row.title, row.entity_type, &args.id[..8]);
        println!("{}", "-".repeat(60));

        println!("\nKeywords:");
        if ontology.keywords.is_empty() { println!("  (none)"); }
        for k in &ontology.keywords { println!("  - {}", k); }

        println!("\nSignal types (technical):");
        if ontology.signal_technical.is_empty() { println!("  (none)"); }
        for t in &ontology.signal_technical { println!("  - {}", t); }

        println!("\nSignal types (social):");
        if ontology.signal_social.is_empty() { println!("  (none)"); }
        for s in &ontology.signal_social { println!("  - {}", s); }

        println!("\nInvolved people:");
        if ontology.people.is_empty() { println!("  (none)"); }
        for p in &ontology.people { println!("  - {}", p); }
    }
...
pub fn run_set(args: OntologySetArgs, json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref kw) = args.add_keywords {
        ...
        ontology_store.add(&args.id, CATEGORY_KEYWORD, &vals, Some("user"))?;
    }
    if let Some(ref ts) = args.add_technical {
        let vals: Vec<&str> = ts.split(',').collect();
        ontology_store.add(&args.id, CATEGORY_SIGNAL_TECHNICAL, &vals, Some("user"))?;
    }
    if let Some(ref ss) = args.add_social {
        let vals: Vec<&str> = ss.split(',').collect();
        ontology_store.add(&args.id, CATEGORY_SIGNAL_SOCIAL, &vals, Some("user"))?;
    }
    if let Some(ref pp) = args.add_people {
        let vals: Vec<&str> = pp.split(',').collect();
        ontology_store.add(&args.id, CATEGORY_PERSON, &vals, Some("user"))?;
    }

    // Removals
    if let Some(ref kw) = args.remove_keywords {
        let vals: Vec<&str> = kw.split(',').collect();
        ontology_store.remove(&args.id, CATEGORY_KEYWORD, &vals)?;
    }
    if let Some(ref pp) = args.remove_people {
        let vals: Vec<&str> = pp.split(',').collect();
        ontology_store.remove(&args.id, CATEGORY_PERSON, &vals)?;
    }
