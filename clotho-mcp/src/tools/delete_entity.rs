...
#[mcp_tool(
    ...
    description = "Delete an entity from all backends (entities.db, content, graph, search index). Accepts full UUID or prefix.",
...
impl DeleteEntityTool {
    pub async fn call_tool(&self) -> Result<CallToolResult, CallToolError> {
        ...
        if let Ok(graph) = GraphStore::open(&ws.graph_path().join("relations.db")) {
            if let Ok(id) = uuid::Uuid::parse_str(&resolved_id).map(EntityId::from) {
        ...
        if let Ok(idx) = SearchIndex::open(&ws.index_path().join("search.db")) {
            let _ = idx.remove_entity(&resolved_id);
        }
