...
pub struct CreateEntityTool {
    /// Entity type (program, responsibility, objective, workstream, task, meeting, transcript, note, reflection, artifact, reference, decision, risk, blocker, question, insight, person)
    ...
    /// URL (for Reference and Artifact — Jira tickets, repos, docs, PRs)
    ...
    pub async fn call_tool(&self) -> Result<CallToolResult, CallToolError> {
        ...
        store
            .insert(&EntityRow {
                ...
                status,
                task_state,
                extraction_status: def_extraction,
        ...
        let graph = GraphStore::open(&ws.graph_path().join("relations.db"))
            .map_err(|e| CallToolError::new(std::io::Error::other(e.to_string())))?;
        graph
            .register_node(&id, et, &self.title)
            .map_err(|e| CallToolError::new(std::io::Error::other(e.to_string())))?;
        if let Some(ref pid) = self.parent_id {
